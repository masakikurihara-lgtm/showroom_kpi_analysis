...
            # SGギフト数率
            mk_sg_gift_df = mksp_df.dropna(subset=['期限あり/期限なしSG総額', 'ギフト数'])
            st.session_state.mk_avg_rate_sg_gift = (mk_sg_gift_df['期限あり/期限なしSG総額'] / mk_sg_gift_df['ギフト数']).mean() * 100 if not mk_sg_gift_df.empty and mk_sg_gift_df['ギフト数'].sum() > 0 else 0
            st.session_state.mk_median_rate_sg_gift = (mk_sg_gift_df['期限あり/期限なしSG総額'] / mk_sg_gift_df['ギフト数']).median() * 100 if not mk_sg_gift_df.empty and mk_sg_gift_df['ギフト数'].sum() > 0 else 0
            
            # SGギフト人数率
            mk_sg_person_df = mksp_df.dropna(subset=['期限あり/期限なしSG人数', 'ギフト人数'])
            st.session_state.mk_avg_rate_sg_person = (mk_sg_person_df['期限あり/期限なしSG人数'] / mk_sg_person_df['ギフト人数']).mean() * 100 if not mk_sg_person_df.empty and mk_sg_person_df['ギフト人数'].sum() > 0 else 0
            st.session_state.mk_median_rate_sg_person = (mk_sg_person_df['期限あり/期限なしSG人数'] / mk_sg_person_df['ギフト人数']).median() * 100 if not mk_sg_person_df.empty and mk_sg_person_df['ギフト人数'].sum() > 0 else 0
...
                # 初見訪問者率
                with col1:
                    first_time_df = df.dropna(subset=['初ルーム来訪者数', '合計視聴数'])
...
                # 初コメント率
                with col2:
                    comment_df = df.dropna(subset=['初コメント人数', 'コメント人数'])
...
                # 初ギフト率
                with col3:
                    gift_df = df.dropna(subset=['初ギフト人数', 'ギフト人数'])
...
                # 短時間滞在者率
                with col5:
                    short_stay_df = df.dropna(subset=['短時間滞在者数', '視聴会員数'])
...
                # SGギフト数率
                with col6:
                    sg_gift_df = df.dropna(subset=['期限あり/期限なしSG総額', 'ギフト数'])
                    total_gifts = sg_gift_df["ギフト数"].sum()
                    total_sg_gifts = sg_gift_df["期限あり/期限なしSG総額"].sum()
...
                # SGギフト人数率
                with col7:
                    sg_person_df = df.dropna(subset=['期限あり/期限なしSG人数', 'ギフト人数'])
                    total_gifters_for_sg = sg_person_df["ギフト人数"].sum()
                    total_sg_person = sg_person_df["期限あり/期限なしSG人数"].sum()
                    sg_person_rate = f"{total_sg_person / total_gifters_for_sg * 100:.1f}%" if total_gifters_for_sg > 0 else "0%"
                    
                    metric_html = f"""
                    <div class="stMetric-container">
