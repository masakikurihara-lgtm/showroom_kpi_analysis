...
                st.info("※ このグラフは、各時間帯に配信した際の各KPIの**平均値**を示しています。突出した値の影響を受けにくく、一般的な傾向を把握するのに役立ちます。棒上の数字は、その時間帯の配信件数です。")
                
...
                    total_viewers_for_first_time = first_time_df["合計視聴数"].sum()
                    first_time_visitors = first_time_df["初ルーム来訪者数"].sum()
                    first_time_rate = f"{first_time_visitors / total_viewers_for_first_time * 100:.1f}%" if total_viewers_for_first_time > 0 else "0%"
                    
                    # MK平均値と中央値
...
                    total_members_for_short_stay = short_stay_df["視聴会員数"].sum()
                    short_stay_visitors = short_stay_df["短時間滞在者数"].sum()
                    short_stay_rate = f"{short_stay_visitors / total_members_for_short_stay * 100:.1f}%" if total_members_for_short_stay > 0 else "0%"
                    
                    # MK平均値と中央値
